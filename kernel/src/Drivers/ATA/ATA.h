...
typedef struct {
    void (*WriteSect)(uint32_t lba, uint8_t buffer[512]);
    void (*ReadSect)(uint32_t lba, uint8_t buffer[512]);
    void (*IdentifyDriveName)(char drive_name[256]);
    uint32_t (*GetTotalSectors)(void);
} DiskDriver;
